...
class ComplexExampleWithParserSpec extends AnyFreeSpec with ComplexExampleFixture {
    ...
    val uniqueTermsMistakesParser = CustomParser
        .idContext[List[HalfParsedTermModel]] { ms =>
            val validTerms = mutable.Set(ms.view.map(_.term):_*)
            CustomParser
    ...
    val modelsParser = Parser.parserParallelWithSequentialEffect[R, String, RawTermModel].applicative.map2(
        termParser
